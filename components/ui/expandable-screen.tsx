      ...
      {isExpanded && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-2">
          ...
          <motion.div
            ...
            layout
            className={`relative flex h-full w-full overflow-y-auto transform-gpu will-change-transform bg-background ${className}`}
          >
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            {showCloseButton && (
              <motion.button
                ...
                transition={{ delay: 0.2 }}
                onClick={collapse}
                className={`absolute right-4 top-4 z-30 flex h-12 w-12 items-center justify-center transition-all rounded-full ${
                  closeButtonClassName ||
                  "text-foreground bg-background border-2 border-border hover:bg-accent hover:scale-110"
                }`}
                aria-label="Close"
              >
                <X className="h-6 w-6" />
